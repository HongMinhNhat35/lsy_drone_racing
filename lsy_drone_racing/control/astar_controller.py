...
class AstarController(Controller):
    ...
    def __init__(self, obs: dict, info: dict, config: Any):
        ...
        self.pid_pos = PIDTracker(0.5, 0.01, 0.05, max_integral=0.1, max_output=1.2)
        self.pid_vel = PIDTracker(0.2, 0.01, 0.03, max_integral=0.1, max_output=0.60)
        self.pid_acc = PIDTracker(0.01, 0.00, 0.0, max_integral=0.01, max_output=0.05)
    ...
    def compute_control(self, obs: dict, info: dict | None = None) -> np.ndarray:
        ...
        # In compute_control, before computing ref_vel:
        curvature = np.linalg.norm(self.spline(t, 2))
        speed_scale = 1.0 / (1.0 + 2.0 * curvature)  # tune the 2.0
